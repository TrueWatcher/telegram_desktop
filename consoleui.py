...
class ConsoleUi(UiBase):
  ...
  def __init__(self) -> None:
    ...
    if self.isLinux == False:
      #print(f"Hi, Windows!")
      # enable ANSI escape sequences for Win 10
  ...
  def inputToCommand(self, line: Union[str,bytes], inv: Inventory) -> Sequence[Union[str,int]]:
    assert self.mode in self.MODES, f"unknown mode {self.mode}"
    dn = self.getCurrentDialog()
    if isinstance(line, bytes):  line2 = line.decode().rstrip('\n\r')
    elif  isinstance(line, str):  line2 = line.rstrip('\n\r')
    else: raise MyException("Wrong LINE type");
    line3 = line2.lower()
  ...
  def presentMessage(self, msg: Message, myid: int, isUnread: bool = False) -> None:
    r = self.repackMessage(msg, myid, isUnread) # , isUnread = False)
    prefix: str = f"\x1b[1;96m{r['prefix']}\x1b[0m" if isUnread else str(r['prefix']) # colorize
