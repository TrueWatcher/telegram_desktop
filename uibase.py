...
class UiBase():
  def __init__(self) -> None:
    ...
    self.dateFormat2 = '%-H:%M' if self.isLinux else '%H:%M'
    self.dateFormat1 = '%b %-d %-H:%M' if self.isLinux else '%b %d %H:%M'
    self.dateFormat0 = '%Y %b %-d %-H:%M' if self.isLinux else '%Y %b %d %H:%M'
  ...
  def localize(self, date) -> str:
    ...
    myformat = self.getDateFormat(dateToLocal)
    #print("'myformat="+myformat+"\n")
    return dateToLocal.strftime(myformat)
